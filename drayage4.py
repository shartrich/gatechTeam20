...
class tripObject:
    ...
    def __init__(self, loadID, portZip, clientZip, shipDate, delivDate, impExp, steamShipLine, equipment, tertiaryDate, portCity = "?", clientCity = "?"):
...
def dataFrameToTripList(df):
    ...
    for idx in range(0, items):
        ...
        try:
            temp = tripObject(row['Id'], row['ShipZip3'], row['ConsZip3'], row['ShipDate'], row['DelivDate'], row['UPPER([Truck Number])'], row['UPPER([Driver])'], row['Equipment'], city1, city2)
...
# Define global Dictionary key variable that will display the distance value b/w two zipCodes
